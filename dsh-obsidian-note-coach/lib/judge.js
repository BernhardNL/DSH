...
export class NoteJudgeService extends Service {
    ...
    threads = new Map();
    constructor(ctx, config, vault, style, writer) {
        super(ctx, 'noteJudge');
        this.config = config;
        this.vault = vault;
        this.style = style;
     ...
     */
    async judge(agent, opinion, context, signal) {
        ...
        let preview;
        try {
            preview = await this.writer.previewNote(agent, index, style, opinion, {
        ...
        if (recordNote) {
            if (!userLocation && !supplement && preview) {
                ...
                insertion = await this.writer.insertNote(agent, index, style, opinion, {
                    suggestedFolder: verdict.suggestedFolder,
                    suggestedTitle: verdict.suggestedTitle,
                    recommendedLocation: verdict.recommendedLocation,
                    userLocation,
     ...
     */
    async followUp(agent, question, signal) {
     ...
     * 放弃) before writing.
     */
    async organizeThread(agent, signal) {
        const thread = this.threads.get(agent.id);
        if (!thread) {
...
function formatSummary(verdict, insertion, supplement) {
...
function normalizeVerdict(parsed) {
    ...
    return {
        ...
        suggestedFolder: typeof parsed?.suggestedFolder === 'string' && parsed.suggestedFolder
            ? parsed.suggestedFolder
            : undefined,
        suggestedTitle: typeof parsed?.suggestedTitle === 'string' && parsed.suggestedTitle
            ? parsed.suggestedTitle
            : undefined,
        recommendedLocation: typeof parsed?.recommendedLocation === 'string' && parsed.recommendedLocation
            ? parsed.recommendedLocation
            : undefined,
        recommendationReason: typeof parsed?.recommendationReason === 'string' && parsed.recommendationReason
...
function pickOrganizeFolder(candidate, folders) {

...
export class NoteReportService extends Service {
    ...
    async generate(agent, req, signal) {
        const root = agent.session.header.cwd;
        if (!root)
            throw new Error('note-coach: no workspace root; select the vault as the workspace first');
            ...
            const walk = async (dir, rel, depth) => {
                ...
                for (const entry of entries) {
                    ...
                    if (entry.type === 'directory') {
                        if (['.git', '.obsidian', '.harness', '.trash', 'node_modules'].includes(entry.name))
                            continue;
        ...
        const markdown = await completeText(this.ctx, agent, {
    ...
    saveReport(root, req, markdown, noteCount, planCount) {
...
function selectNotes(notes, req) {
    const matches = (n) => {

 ...
 *   comparison).
 */
export function registerTools(ctx, config, judge, report) {
    const disposers = [];
    if (config.autoDetect) {
        disposers.push(ctx.tools.register(defineTool({
            name: 'note_coach_judge',
            description: '当用户表达观点或看法时（例如“我认为…”“我觉得…”“我的看法是…”）调用。独立、中立地判断该观点是否正确（对照用户自己的知识库笔记），输出分开的【判断】【理由】【建议】和【推荐记录位置】；若判断为正确或属非主观陈述，则弹窗征询用户：同意按推荐位置记录，或用自然语言补充内容/指定位置。观点判断不依赖任何历史价值记忆。',
            parameters: {
        ...
        disposers.push(ctx.tools.register(defineTool({
            ...
            description: '当用户对上一次观点判断的理由/结论继续追问时调用（例如“为什么这么判断？”“能举个反例吗？”“那如果换成XX呢？”）。基于原始观点与先前的判断、理由连续作答，不依赖历史价值记忆。',
            parameters: {
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        description: '生成笔记报告：对指定内容或整个知识库做分类总结（纯内容总结）或客观评价（结构/覆盖/质量/缺口，中立语气）；笔记中含计划类内容（frontmatter type: plan 或含清单）时自动附带“计划 vs 现状”对比。报告保存到 .harness/reports/ 并返回 Markdown。',
        parameters: {
            scope: {
                type: 'string',
                required: true,
        ...
        execute: async (args, exec) => {
            ...
            try {
                const result = await report.generate(exec.agent, { scope: args.scope, target: args.target, mode: args.mode, category: args.category }, exec.signal);

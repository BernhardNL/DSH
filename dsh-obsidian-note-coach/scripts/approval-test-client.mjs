...
ws.addEventListener('open', () => {
  console.log('[mux] connected');
  // Trigger a new opinion likely to be judged "correct".
  const text =
    '用文件夹+frontmatter+列表式笔记的组合管理知识，比单一维度的组织方式更可靠。';
  rpc('session.prompt', {
    sessionId: process.argv[2],
    mode: 'queue',
    content: [{ type: 'text', text }],
  }).then((resp) => console.log('[prompt]', JSON.stringify(resp.result).slice(0, 200)));
});

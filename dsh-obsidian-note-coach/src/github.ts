...
export class NoteGitService extends Service {
  ...
  private async loadState(root: string): Promise<GitState> {
    try {
  ...
  private async saveState(root: string, state: GitState): Promise<void> {
    const target = await this.ctx.fs.resolve(this.statePath(root));
    await this.ctx.fs.writeText(target, JSON.stringify(state, null, 2) + '\n');
  }

  /** Record the target repository URL. */
  async setRepo(root: string, url: string): Promise<string> {
  ...
  async push(root: string, message?: string, signal?: AbortSignal): Promise<PushResult> {
    ...
    const out: string[] = [];
    const run = async (args: string[], opts?: { cwd?: string }): Promise<string> => {

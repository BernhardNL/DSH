...
const en: Record<string, string> = {
  ...
  'page.urlHint': 'Dedicated page · new window · bookmarkable',
...
const PAGE_PATH = '/notes-coach';
const PAGE_HASH = '#/notes-coach';

function isPageOpen(): boolean {
  return window.location.pathname === PAGE_PATH || window.location.hash === PAGE_HASH;
}

/** Page-open state synced to the URL (path or hash). */
function usePageOpen(): [boolean, () => void] {
  ...
  const close = () => {
    // Came via same-window navigation (back button restores the main app);
    // otherwise (direct visit / bookmark) return to the root.
          ...
          jsxs('div', {
            ...
            children: [
              jsxs('div', { style: { display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }, children: [
                ...
                jsx('span', { style: { fontSize: 12, color: 'var(--dsh-muted, #888)' }, children: `${t('page.urlHint')} · ${window.location.origin}${PAGE_PATH}` }),
...
function SidebarEntry({
  ...
  const openPage = () => {
    if (open) return;
    // Navigate the CURRENT window to the dedicated URL (no second window):
    // the address genuinely changes to /notes-coach and the browser back
    // button returns to the main app.

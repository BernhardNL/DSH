...
export class CodeGenService extends Service {
  ...
  async generate(agent: Agent, root: string, request: string, signal?: AbortSignal): Promise<string> {
    ...
    const parsedJson = extractJson<{ fileName?: string; code?: string; description?: string }>(answer);
    const code = fenced?.[1]?.trim() || parsedJson?.code;
    if (!code) throw new Error('math-assistant: 代码生成失败（未返回有效代码）。');

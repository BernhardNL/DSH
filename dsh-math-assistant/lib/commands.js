...
export function registerCommands(ctx, config, memory, analysis, pipeline, dataSvc, codegen, refs, paper, format) {
    const disposers = [];
    const wrap = (fn) => async ({ agent, rawInput, signal }) => {
        try {
    ...
    disposers.push(ctx.commands.register({
        ...
        handler: wrap(async (agent, input, signal) => {
            ...
            if (!trimmed)
                return '用法：/math-problem <题目描述 或 文件名>。文件需先放入当前工作区。';
            ...
            const summary = `✅ 题目已设定，完成分析（${result.length} 个问题）：\n\n` +
    ...
    disposers.push(ctx.commands.register({
        ...
        handler: wrap(async (agent, input, signal) => {
            ...
            if (!file)
                return '用法：/math-repaper <源论文文件名> [额外要求]';
    ...
    disposers.push(ctx.commands.register({
        ...
        handler: wrap(async (agent, input, signal) => {
            ...
            if (!files.length)
                return '用法：/math-learn-format <参考论文1> [参考论文2] …';

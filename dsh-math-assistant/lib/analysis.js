...
export class AnalysisService extends Service {
    ...
    async analyze(agent, root, signal) {
        ...
        const answer = await completeText(this.ctx, agent, ANALYZE_SYSTEM, user, { temperature: this.config.llm.temperature, maxTokens: this.config.llm.maxTokens }, signal);
...
function buildProblemText(mem) {
    ...
    if (mem.problem.description)
        parts.push(mem.problem.description);
    return parts.join('\n\n');
}

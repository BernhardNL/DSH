...
export class FormatService extends Service {
    ...
    async learn(agent, root, files, signal) {
        if (!files.length)
            throw new Error('请至少提供一个参考论文文件。');
        ...
        if (!samples.length)
            throw new Error('无法读取任何参考论文。请确认文件在工作区且为 docx/pdf/txt 格式。');
        const answer = await completeText(this.ctx, agent, FORMAT_SYSTEM, `Exemplar papers:\n\n${samples.join('\n\n---\n\n')}\n\nExtract the format conventions now.`, { temperature: 0.2, maxTokens: 2000 }, signal);

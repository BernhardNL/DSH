...
export class PipelineService extends Service {
     ...
     */
    async run(agent, root, command, signal) {
        const mem = await this.memory.load(root);
        const context = buildContext(mem);
        // 1. 规划
        const planAnswer = await completeText(this.ctx, agent, PLAN_SYSTEM, `Project context:\n${context}\n\nUser command: ${command}\n\nDesign the pipeline now.`, { temperature: 0.2, maxTokens: 2000 }, signal);
        const plan = extractJson(planAnswer);
        ...
        for (let i = 0; i < stages.length; i++) {
            ...
            try {
                ...
                output = await completeText(this.ctx, agent, WORKER_SYSTEM, `Project context:\n${workerContext}\n\nYour task (stage ${i + 1}): ${stage.title}\n${stage.reasoningBrief}\n\nProduce the deliverable now.`, { temperature: this.config.llm.temperature, maxTokens: this.config.llm.maxTokens }, signal);
    ...
    async saveAndValidate(agent, root, stageTitle, code, signal) {
        try {

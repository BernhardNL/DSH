...
export class PaperService extends Service {
    ...
    async generate(agent, root, extraReqs, signal) {
        ...
        const paperMd = await completeText(this.ctx, agent, PAPER_SYSTEM, `Project context:\n${context}${profileText ? `\n\n${profileText}` : ''}${modReq}\n\nWrite the paper now.`, { temperature: 0.4, maxTokens: 6000 }, signal);
        if (!paperMd.trim()) {
        ...
        return `✅ 论文已生成：\n- Word：${docxPath}\n- Markdown 底稿：${mdPath}\n\n（公式已转为 Word 原生公式，统计图自动嵌入。可在对话框提出修改要求重新生成。）${thinContext
     ...
     */
    async repaper(agent, root, sourceFile, extraReqs, signal) {
        ...
        if (occurrences.length > 0) {
            ...
            for (let i = 0; i < occurrences.length; i++) {
                ...
                const out = await completeText(this.ctx, agent, FORMULA_SYSTEM, `The paper has this incomplete-formula marker at position ${i + 1}/${occurrences.length}:
...
Output STRICT JSON: { "latex": string }`, { temperature: 0.2, maxTokens: 800 }, signal);
     ...
     */
    async repaperDocx(root, sourceFile, latexByOccurrence, markerCount, extraReqs, signal) {
        ...
        if (markerCount === 0) {
            ...
            result = await patchDocxFormulas(root, this.config.workDir, sourcePath, docxPath, latexByOccurrence.map((l) => l || null), this.config.pythonPath, headingRestyle);
        }
        if (!result.ok) {
            return `⚠️ docx 原位重排失败：${result.error}（源文件未改动，未生成输出）。`;
        }
        const note = markerCount > 0
        ...
        if (result.tables)
            preservedParts.push(`${result.tables} 个表格`);
        if (result.media)
            preservedParts.push(`${result.media} 张图片`);
    ...
    async reformatAll(agent, root, md, profile, signal) {
        ...
        for (let i = 0; i < allChunks.length; i++) {
            ...
            let out = '';
            try {
                out = await completeText(this.ctx, agent, REFORMAT_SYSTEM, `Format requirements:\n${profileText}\n\nDocument chunk ${i + 1}/${allChunks.length} to reformat:\n\n---\n${allChunks[i]}\n\nReformat this chunk to follow the format requirements. Output only the reformatted chunk.`, { temperature: 0.2, maxTokens: 8000 }, signal);
            }
            catch (err) {
    ...
    async selfCheck(agent, root, md, result, signal) {
        if (result.aborted) {
        ...
        if (open % 2 !== 0)
            issues.push(`公式定界符 $$ 数量为奇数（${open}），可能存在未闭合公式`);
        ...
        for (const idx of sampleIdx) {
            const out = await completeText(this.ctx, agent, CHECK_SYSTEM, `Reformatted document chunk ${idx + 1}/${chunks.length} to inspect:\n\n---\n${chunks[idx].slice(0, 6000)}\n\nInspect this chunk now.`, { temperature: 0.1, maxTokens: 600 }, signal);
            const parsed = extractJson(out);
            const chunkIssues = parsed?.issues ?? [];
            if (parsed && parsed.ok === false)
                issues.push(`第 ${idx + 1} 块：${chunkIssues.slice(0, 3).join('；') || '内容疑似不完整'}`);
            else if (parsed && Array.isArray(chunkIssues) && chunkIssues.length)
                issues.push(`第 ${idx + 1} 块：${chunkIssues.slice(0, 2).join('；')}`);
        }
        return { ok: issues.length === 0, issues };
    }
    async saveCheckReport(root, sourceFile, check) {
        try {
            const { mkdirSync, writeFileSync } = await import('node:fs');
            const paperDir = root.endsWith('/') ? root + this.config.paperDir : `${root}/${this.config.paperDir}`;
            mkdirSync(paperDir, { recursive: true });
...
function buildPaperContext(mem) {
    const parts = [];
    if (mem.problem.title)
        parts.push(`题目：${mem.problem.title}`);
    // 大段原始内容截断，避免超长 prompt 触发空响应
    if (mem.problem.description)
        parts.push(`问题描述：${mem.problem.description.slice(0, 4000)}${mem.problem.description.length > 4000 ? '\n…（截断）' : ''}`);
    if (mem.problem.problemFile)
        parts.push(`问题文件：${mem.problem.problemFile}`);
...
async function convertToDocx(projectRoot, workDir, mdPath, docxPath, configuredPython) {
    const convertCode = [
        'import sys',
...
function markdownify(text) {
    return text
        ...
        .map((line) => {
        const t = line.trim();
        if (/^第[一二三四五六七八九十]+[章节部分]/.test(t) || /^(摘\s*要|关键词|目\s*录|参考文献)$/.test(t))
            return `# ${t}`;
        if (/^\d+\s+\|/.test(t))
            return line; // 表格/列表行不转标题
        if (/^\d+\.\d+\.\d+\s+\S/.test(t))
            return `### ${t}`;
        if (/^\d+\.\d+\s+\S/.test(t))
            return `## ${t}`;
...
function findFormulaOccurrences(text) {
    const out = [];
...
/** 渲染格式档案为提示文本。 */
function formatProfileToPrompt(p) {
    return [
        '目标论文格式规范（必须遵守）：',
...
function splitByHeadings(text, size) {
...
/** 请求取消某项目的重排。 */
export function requestCancel(root) {
    cancelRequests.add(root);
}
/** 清空取消标记（重排完成/取消后自动调用，也可手动）。 */
export function clearCancel(root) {
    cancelRequests.delete(root);
}
const CHECK_SYSTEM = [
...
function uniqueSample(n, count) {
    ...
    for (let i = step; i < n - 1 && picks.size < count; i += step)
        picks.add(i);
